...
import { FileSpreadsheet, Download, Filter, FileText, CheckCircle2, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-extrabold text-slate-900">Hydrology Analytics & Statewide Reports</h2>
          </div>
          <p className="text-xs text-slate-500">Generate executive PDF & Excel reports for Gujarat Water Resources Ministry</p>
        </div>

        <button
          onClick={handleDownload}
          disabled={downloading}
          className="px-4 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs rounded-xl shadow-md flex items-center space-x-2 shrink-0"
        >
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
        <h3 className="text-base font-bold text-slate-900">District Supply vs Demand Discrepancy (MLD)</h3>
        ...
        <div className="h-80 w-full pt-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <Bar dataKey="Supply" fill="#0284c7" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        {[
          ...
          <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] text-slate-400 font-mono font-bold">{report.date}</span>
            </div>
            <h4 className="font-bold text-slate-900 text-sm">{report.title}</h4>
            <p className="text-xs text-slate-500">{report.desc}</p>
            <button
              onClick={handleDownload}
              className="w-full py-2 bg-slate-50 hover:bg-sky-50 text-sky-800 border border-slate-200 hover:border-sky-300 font-bold text-xs rounded-xl transition-colors"
            >
              Generate Report
            </button>
          </div>
