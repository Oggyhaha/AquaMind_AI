...
import { Bot, Send, Sparkles, X, User, CornerDownLeft, RefreshCw, Layers } from 'lucide-react';
...
export const AIChatDrawer: React.FC<AIChatDrawerProps> = ({
  ...
  // Update initial message when userRole changes
  useEffect(() => {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    // AI Response generation adapted to role
    setTimeout(() => {
      let responseText = "";
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[420px] bg-white shadow-2xl border-l border-slate-200 z-50 flex flex-col animate-in slide-in-from-right duration-300">
      ...
      {/* Header */}
      <div className="p-4 bg-gradient-to-r from-slate-900 via-sky-950 to-blue-900 text-white flex items-center justify-between shadow-md">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold">AquaMind Role-Adapted AI</h3>
            <p className="text-[10px] text-cyan-300 font-mono">Tailored for: {userRole.replace('_', ' ').toUpperCase()}</p>
          </div>
        </div>

        <button onClick={onClose} className="p-1 rounded-lg hover:bg-white/10 text-slate-300 hover:text-white">
          <X className="w-5 h-5" />
        </button>
      </div>
        ...
        {messages.map((msg) => (
          ...
          >
            <div className={`max-w-[85%] rounded-2xl p-3.5 text-xs shadow-xs space-y-1 ${
              ...
              {msg.roleBadge && (
                <span className="text-[9px] px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-900 font-bold uppercase tracking-wider block w-max">
                  {msg.roleBadge}
                </span>
              )}
              <p className="leading-relaxed">{msg.text}</p>
              <span className={`text-[9px] block text-right font-mono ${msg.sender === 'user' ? 'text-sky-200' : 'text-slate-400'}`}>
                {msg.timestamp}
              </span>
            </div>
          </div>
      ...
      <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-200 flex items-center space-x-2">
        <input
          ...
          className="flex-1 p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-sky-500 focus:outline-none"
        />
        <button
          type="submit"
          className="p-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-xl shadow-md transition-all shrink-0"
        >
